...
// Prevents API calls on every keystroke for better performance
...
const FilterPanel = ({ filters, setFilters }) => {
  ...
  // Hardcoded options based on your screenshot.
  // For a more robust solution, these could be fetched from Strapi.
  const filterOptions = {
...
// ##################################################################
// # 2. CAR LIST COMPONENT (MODIFIED to accept filters)
// ##################################################################
...
// The main CarList component, now accepting a `filters` prop
const CarList = ({ filters }) => {
  ...
  // Use the debounced search term to avoid rapid API calls
  ...
  const fetchCars = useCallback(async (pageToFetch, currentFilters) => {
    ...
    setLoading(true);
    try {
      // **MODIFIED PART**: Construct Strapi filter query
      const strapiFilters = {
        $and: [], // Use $and to combine all filter conditions
      };

      // 1. Text search filter for 'marke' or 'modell'
      ...
      // 2. Checkbox filters (enumeration fields)
      ["autokategorien", "fahrzeugtypen", "spezifikationen"].forEach(
        (category) => {
          if (currentFilters[category] && currentFilters[category].length > 0) {
            strapiFilters.$and.push({
              [category]: { $in: currentFilters[category] },
            });
          }
        }
      );
      ...
      const query = qs.stringify(
        {
          ...
          filters: strapiFilters.$and.length > 0 ? strapiFilters : undefined,
        },
  ...
  // Effect to reset page number and trigger refetch when filters change
  useEffect(() => {
  ...
  // Effect to fetch cars when page or filters change
  useEffect(() => {
...
// ##################################################################
// # 3. PARENT CONTAINER (Manages state and layout)
// ##################################################################
